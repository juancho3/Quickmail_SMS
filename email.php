...
if (!empty($type)) {
    ...
    $email->sms = optional_param('sms', '', PARAM_TEXT);
    ...
    if (empty($warnings)) {
        ...
        if (isset($data->send)) {
            ...
            foreach (explode(',', $data->mailto) as $userid) {
                ...
                // Send a special mail to SMS gateway
                if (!empty($data->sms) ) {
                    $phone1 = $everyone[$userid]->phone1;
                    $phone2 = $everyone[$userid]->phone2;

                    // Prepare GMS message with allowed characters, rest is replaced with an equivalent
                    $gsm_orig_message = $config['smsprefix'] . $data->sms;
                    $gsm_message_utf8 = substr( $gsm_orig_message, 0, 150 );
                    $gsm_message_latin = utf8_decode( $gsm_message_utf8 );
                    // Remove unwanted chars
                    $len = strlen( $gsm_message_latin );
                    $gsm_message = '';
                    for( $i=0; $i<$len; $i++ ) {
                        $nchar = substr($gsm_message_latin,$i,1);
                        if( ctype_alnum($nchar) )                  $gsm_message .= $nchar;
                        else if( strpbrk( $nchar, ' ?,.-;:_+' ) )  $gsm_message .= $nchar;
                        else if( $nchar == 'á' ) $gsm_message .= 'a';
                        else if( $nchar == 'é' ) $gsm_message .= 'e';
                        else if( $nchar == 'í' ) $gsm_message .= 'i';
                        else if( $nchar == 'ó' ) $gsm_message .= 'o';
                        else if( $nchar == 'ú' ) $gsm_message .= 'u';
                        else if( $nchar == 'Á' ) $gsm_message .= 'A';
                        else if( $nchar == 'É' ) $gsm_message .= 'E';
                        else if( $nchar == 'Í' ) $gsm_message .= 'I';
                        else if( $nchar == 'Ó' ) $gsm_message .= 'O';
                        else if( $nchar == 'Ú' ) $gsm_message .= 'U';
                        else if( $nchar == 'ñ' ) $gsm_message .= 'n';
                        else if( $nchar == 'Ñ' ) $gsm_message .= 'N';
                    }

                    if (!empty($phone1) and strlen($phone1)==9 and $phone1{0}=='6' ) {
                        $user_sms = clone( $everyone[$userid] );
                        $user_sms->email = $config['smsmail'];
                        $success = email_to_user( $user_sms, $user, $phone1, $gsm_message );
                    }
                    if (!empty($phone2) and strlen($phone2)==9 and $phone2{0}=='6' ) {
                        $user_sms = clone( $everyone[$userid] );
                        $user_sms->email = $config['smsmail'];
                        $success = email_to_user( $user_sms, $user, $phone2, $gsm_message );
                    }
                }
